...
1. Put the entity in `api.simplified.skyblock.model`, implementing `JpaModel`.
...
- **Schema check** - required when your change adds or renames a column:
  ...
  ```bash
  ./gradlew test --tests '*SchemaExporter*' || java -cp <test-classpath> api.simplified.skyblock.schema.SchemaExporter
  ```
